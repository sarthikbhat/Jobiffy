...
class Classifier:
    ...
    def create_profile(self,file):
        ...
        ML_words = [nlp(text) for text in keyword_dict['machinelearning'].dropna(axis = 0)]
        DL_words = [nlp(text) for text in keyword_dict['deeplearning'].dropna(axis = 0)]
        ...
        Data_Engineer_words = [nlp(text) for text in keyword_dict['dataengineer'].dropna(axis = 0)]
        Blockchain_words=[nlp(text) for text in keyword_dict['blockchain'].dropna(axis = 0)]
        Frontend_words=[nlp(text) for text in keyword_dict['frontend'].dropna(axis = 0)]
        Backend_words=[nlp(text) for text in keyword_dict['backend'].dropna(axis = 0)]
        ...
        Prog_words=[nlp(text) for text in keyword_dict['programmer'].dropna(axis = 0)]
